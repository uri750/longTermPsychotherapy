...
def analyze_transcript(transcript):
    ...
    אסור לקודד תור דיבור שלם כמקשה אחת אם יש בו שינוי נושא. עליך לפצל כל תור דיבור למקטעים קצרים (משפטים/חלקי משפטים) בכל מעבר בין דיבור על העבר/מחוץ לחדר, לבין חוויה בחדר.
    
    שלב 2: סינון Ex-room ושאלות
    אם תת-המקטע מתאר אירוע שקרה מחוץ לחדר, תבניות כלליות, או שהוא שאלה - החזר תמיד NONE.
    *חריג קריטי:* הבעות לא-מילוליות בחדר (כמו בכי, תנועות גוף) או חוויה עכשווית בחדר שמתייחסת לעבר (למשל: "אני לא מבין עכשיו לאן הזמן עף") - הן "כאן ועכשיו" ויש לקודד אותן, לא NONE!
    
    שלב 3: זיהוי וקידוד "כאן ועכשיו"
    אם תת-המקטע עוסק במה שקורה כעת בחדר, הרכב קוד של 3 אותיות באנגלית (למשל P-T-C):
    
    אות ראשונה - הדובר: 'T' (המטפל) או 'P' (המטופל).
    
    אות שנייה - המוקד (דגשים קריטיים):
    - 'T': המוקד הוא המטפל. (למשל: כשהמטופל אומר "אתה יציב" או "אתה לא מעריץ אותי").
    - 'P': המוקד הוא המטופל. (למשל: כשהמטופל מנתח את מניעיו שלו, "רציתי שתעריץ אותי", או כשהמטפל מתייחס לפעולת המטופל, "התחלת להגיד").
    - 'D': דיאדה. שמור אך ורק לחוויות משותפות וממוזגות שלא ניתנות להפרדה ("שנינו מרגישים", "השתיקה שלנו"). התייחסות של המטפל למשהו שהמטופל אמר הרגע היא *לא* דיאדה.
    
    אות שלישית - הממד:
    - 'S': מרחב (Space - פוטנציאל לחוות. למשל "אני חסום").
    - 'C': תוכן (Content - החוויה עצמה: רגש, מחשבה, הבעה לא-מילולית כמו בכי או תזוזה).
    - 'O': סדר (Order - ספקות, שקילת חלופות, קונפליקט פנימי, למשל "מצד אחד... מצד שני").
