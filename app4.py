...
def analyze_transcript(transcript):
    ...
    אם המקטע עוסק במה שקורה כעת בחדר בין המטפל למטופל, הרכב קוד של 3 אותיות באנגלית המופרדות במקף (למשל T-P-C או P-D-S) לפי הלוגיקה הנוקשה הבאה:
    ...
    אות שלישית - הממד (Dimension - מהות החוויה המובעת):
    - 'S': מרחב (Space - הפוטנציאל לחוות. מתייחס ליכולת או חוסר היכולת להרגיש/לחשוב, למשל "אני חסום", "ריק לי", "אני לא מסוגל לחשוב").
    - 'C': תוכן (Content - החוויה עצמה: מחשבה ספציפית, רגש ספציפי או תחושה/פעולה מסוימת בחדר, למשל "אני עצוב", "אני רועד").
    - 'O': סדר (Order - היחסים בין חוויות: ספקות, קונפליקטים, דילמות פנימיות, למשל "מצד אחד... מצד שני").
    ...
    חשוב: בתוך ה-"explanation", נמק קצרות בעברית מדוע בחרת בכל אחת מ-3 האותיות (דובר, מוקד, ממד) על בסיס חוקי המטריקס.
    """
