...
def analyze_transcript(transcript):
    ...
    פצל כל תור דיבור למקטעים קצרים. **חוק ההורשה:** כשאתה מפצל פסקה של מטופל למספר מקטעים, חובה עליך "להוריש" את זהות הדובר לכולם. עליך להוסיף את הקידומת המקורית ("אני:" או "הוא:") באופן יזום לתחילת כל תת-מקטע שנוצר מאותו תור דיבור, כדי שלא תאבד את ההקשר. טקסטים בסוגריים שייכים לדובר האחרון שצוין.
    ...
    *חריגים קריטיים (זה 'כאן ועכשיו' וחובה לקודד אותם, לא NONE!):*
    1. רפלקציה על הקשר הטיפולי: כשהמטופל מנתח את התנהגותו/מניעיו כלפי המטפל בחדר.
    2. מחשבה/רגש בהווה: תיאור חוויה שמתרחשת עכשיו ("אני לא מבין עכשיו לאן הזמן עף").
    3. הבעות לא-מילוליות בחדר: בכי, תנועות גוף, חיוך.
    
    שלב 3: זיהוי וקידוד "כאן ועכשיו"
    למקטעים שאינם NONE, הרכב קוד של 3 אותיות באנגלית:
    
    אות ראשונה - הדובר: 
    - 'T' - על סמך הקידומת "אני:".
    - 'P' - על סמך הקידומת "הוא:".
    
    אות שנייה - המוקד:
    - 'T': המטפל (למשל: המטופל מדבר על תכונות/פעולות המטפל).
    - 'P': המטופל (למשל: המטופל מדבר על עצמו/מניעיו, או כשהמטפל מתייחס למטופל).
    - 'D': דיאדה (רק חוויות משותפות וממוזגות לחלוטין - "שנינו").
    
    אות שלישית - הממד:
    - 'S': מרחב (Space - פוטנציאל לחוות. למשל: "אני חסום", "ריק לי", חוסר יכולת למלל חוויה).
    - 'C': תוכן (Content - החוויה עצמה: רגש, מחשבה, הבעה לא-מילולית, מתן הוראה בחדר).
    - 'O': סדר (Order - ספקות, דילמות פנימיות, קונפליקט).
    *כלל תעדוף:* אם הליבה של המשפט היא חוסר מסוגלות לחוות/למלל - קודד S.
    ...
    "text" (תת-המקטע הכולל את קידומת הדובר המקורית או המורשת), "code" (הקוד או NONE), "explanation" (הסבר קצר בעברית).
    """
