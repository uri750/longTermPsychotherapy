...
API_KEY = "AIzaSyALUU58wHsK7c__PHFITKBlb_1rtH2XrME" 
...
def analyze_transcript(transcript):
    ...
    1. עבר ומחוץ לחדר: דברים שקרו לפני תחילת הפגישה, כולל בדרך לקליניקה ("חשבתי בדרך"), או תקשורת קודמת בין המטפל למטופל ("התעקשת שאבוא", הודעות). זה Ex-room!
    ...
    רק למקטעים שאינם NONE ושמתרחשים *כרגע* בחדר (כמו מתן הוראה פיזית "תתרחק", או חוויה שמתרחשת הרגע), הרכב קוד של 3 אותיות:
    ...
    - 'P': המטופל (המטופל מדבר על עצמו).
    - 'D': דיאדה (רק חוויות משותפות ממוזגות לחלוטין).
    
    אות שלישית - הממד:
    - 'S': מרחב (Space - חוסר יכולת לחוות/למלל - "אני חסום").
    - 'C': תוכן (Content - החוויה/הפעולה הנוכחית, למשל הוראה פיזית, רגש שעולה כעת).
    - 'O': סדר (Order - דילמות, ספקות, קונפליקט).
