...
def analyze_transcript(transcript):
    ...
    פצל כל תור דיבור למקטעים קצרים (משפטים/חלקי משפטים). חובה לפצל במיוחד במעברים בין סיפורי רקע מבחוץ (Ex-room) לבין התייחסות למטפל, לקשר הטיפולי או לרגשות שעולים כעת בחדר.
    ...
    *חריגים קריטיים (זה 'כאן ועכשיו' וחובה לקודד אותם, לא NONE!):*
    1. רפלקציה על הקשר הטיפולי והמניעים: כשהמטופל מנתח את ההתנהגות שלו כלפי המטפל בחדר ("אני מסכים שרציתי שתעריץ אותי") - זה תמיד מקודד!
    ...
    אות שנייה - המוקד:
    - 'T': המטפל (כשהמטופל מדבר על תכונות המטפל עצמו).
    - 'P': המטופל. **כלל ברזל:** כאשר המטופל מדבר על המניעים *שלו עצמו* או הפעולות *שלו* כלפי המטפל (למשל: "רציתי שתעריץ", "רציתי להראות לך") - המוקד הוא תמיד P ולא T!
    - 'D': דיאדה (רק חוויות משותפות וממוזגות לחלוטין - "שנינו").
    
    אות שלישית - הממד (שים לב לתעדוף במשפטים מורכבים):
    - 'S': מרחב (Space - הפוטנציאל לחוות. מצביע על יכולת או חוסר יכולת להרגיש/לחשוב. למשל: "אני חסום", "ריק לי", או חוסר יכולת למלל חוויה - "אני לא מצליח להעביר במילים").
    - 'C': תוכן (Content - החוויה עצמה: רגש ספציפי, מחשבה, פעולה).
    - 'O': סדר (Order - ספקות, דילמות פנימיות, קונפליקט, "מצד אחד... מצד שני").
    *כלל תעדוף:* אם משפט מכיל כמה ממדים (למשל פעולה שנובעת מחוסר יכולת מילולית), קודד לפי נושא הליבה. אם הליבה היא חוסר המסוגלות - קודד S.
